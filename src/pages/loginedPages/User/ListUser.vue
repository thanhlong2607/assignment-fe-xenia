...
const headers = [
  {
    value: "firstName",
    label: "user.tableDL.first_name",
  },
  {
    value: "lastName",
    label: "user.tableDL.last_name",
  },
  {
    value: "dob",
    label: "user.tableDL.dob",
  },
  {
    value: "email",
    label: "user.tableDL.email",
  },
  {
    value: "phoneCountryCode",
    label: "user.tableDL.country_code",
  },
  {
    value: "phoneNumber",
    label: "user.tableDL.phone_number",
  },
  {
    value: "status",
    label: "user.tableDL.status",
  },
];
const customColumn = {
...
const eventFunc = (e) => {
  if (e.name === "view") {
    ...
    ElMessageBox.confirm("Are you sure to remove this one?")
      .then(() => {
        store.dispatch("removeUser", e.item.id);
      })
      .catch(() => {});
  }
};
